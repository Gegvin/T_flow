...
fn test_valid_file_parses_correctly() {
    require_binary();
    assert!(!file_has_errors("examples/valid.tflow"), "Valid file should have no errors");
}
...
fn test_error_missing_semicolon() {
    require_binary();
    assert!(file_has_errors("examples/error_missing_semicolon.tflow"), "Missing semicolon should be detected");
}

#[test]
fn test_error_if_in_node() {
    require_binary();
    assert!(file_has_errors("examples/error_if_in_node.tflow"), "If statement in node should be detected");
}

#[test]
fn test_error_let_in_step() {
    require_binary();
    assert!(file_has_errors("examples/error_let_in_stap.tflow"), "Let statement in step should be detected");
}

#[test]
fn test_error_missing_closing_brace_for_node() {
    require_binary();
    assert!(file_has_errors("examples/error_missing_closing_brace_for_node.tflow"), "Missing closing brace should be detected");
}

#[test]
fn test_error_missing_expression_after_operator() {
    require_binary();
    assert!(file_has_errors("examples/error_missing_expression_after_operator.tflow"), "Missing expression should be detected");
}

#[test]
fn test_error_missing_parentheses_after_function_name() {
    require_binary();
    assert!(file_has_errors("examples/error_missing_parentheses_after_function_name.tflow"), "Missing parentheses should be detected");
}

#[test]
fn test_error_missing_return_type_in_function() {
    require_binary();
    assert!(file_has_errors("examples/error_missing_return_type_in_function.tflow"), "Missing return type should be detected");
}

#[test]
fn test_error_unexpected_symbol() {
    require_binary();
    assert!(file_has_errors("examples/error_unexpected_symbol.tflow"), "Unexpected symbol should be detected");
}
