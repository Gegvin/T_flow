...
После этого откроется новое окно **Extension Development Host**. В нём откройте файл из папки `examples/` и проверьте работу расширения.
...
После открытия файла `.tflow` VS Code автоматически определяет его как файл нашего супер крутого языка.
